...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<CreatedResp | ErrorResp>
) {
  // allow only POST
  if (req.method !== "POST") {
  ...
  const { size: rawSize, color: rawColor, quantity: rawQty, status: rawStatus } =
    req.body as { size?: unknown; color?: unknown; quantity?: unknown; status?: unknown };
  if (
    typeof rawSize !== "string" ||
    typeof rawColor !== "string" ||
    typeof rawQty !== "number" ||
    rawQty < 0 ||
    !["available", "unavailable", "discontinued"].includes(rawStatus as string)
  ) {
  ...
  try {
    // transaction: create variant + inventory
